...
void __fastcall TForm1::Button3Click(TObject *Sender)
{
	...
	if(field->begin->posX == -1 || field->end->posX == -2) {
		Application->MessageBox(String("Установите начальную и конечную точки").c_str(),
		 String("Лабиринт").c_str(), NULL);
		return;
	}
	bool isUse = isUseDijkstra(field);
	bool answer = isUse ? Dijkstra(field) : Moore(field);
	String message = isUse? "Клетки имеют разную проходимость. Будет использован алгоритм Дейкстры":
							"Клетки имеют одинаковую проходимость. Будет использован алгоритм Мура";
	Application->MessageBox(message.c_str(), String("Лабиринт").c_str(), NULL);
	if(answer == false)
		 Application->MessageBox(String("Путь не найден").c_str(),
		 String("Лабиринт").c_str(), NULL);
	else {
...
void __fastcall TForm1::Button1Click(TObject *Sender)
{
	try {
		...
		if( horizontalNumber < 1 || verticalNumber < 1) {
			Application->MessageBox(String("Размеры поля введены неверно").c_str(),
			String("Лабиринт").c_str(), NULL);
			return;
		}
		if( horizontalNumber > 50 || verticalNumber > 50) {
			Application->MessageBox(String("Слишком большой размер поля (>50). Введите заново").c_str(),
			String("Лабиринт").c_str(), NULL);
			return;
		}
		delete field;
		...
		Application->MessageBox(String("Не введены поля размеров лабиринта").c_str(),
		 String("Лабиринт").c_str(), NULL);
	}
}
//---------------------------------------------------------------------------
